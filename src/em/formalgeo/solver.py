...
def test_problem():
    ...
    problem.construct('Line(x):PointOnLine(A,x)&EqualAngle(b,x,x,c)')
    problem.construct('Line(y):PointOnLine(B,y)&EqualAngle(c,y,y,a)')
    ...
    problem.construct('Circle(O):PointOnCircle(A,O)&PointOnCircle(B,O)&PointOnCircle(C,O)')
