...
def generate_theorem_dataset(file_path, output_path, index):
    ...
    for fid in all_produced_facts:
        ...
        # B. 类型过滤：检查fact是否在黑名单中
        fact_content = notes[fid]
        predicate = fact_content[0]
        if predicate in IGNORED_GOAL_TYPES:
            continue

        reasoning_facts.append(fid)

    print(f"找到 {len(reasoning_facts)} 个有效目标 (已排除 {IGNORED_GOAL_TYPES})。")

    # --- 2. 生成数据 ---
    for goal_fid in reasoning_facts:
...
if __name__ == "__main__":
    ...
    generate_theorem_dataset(
        ...
        'tp_data/problem_2_reasoning.json',
        constructions_len
    )

    generate_theorem_dataset_multiple(
        'data/hypergraph-2-solve.json',
        'tp_data/problem_2_multiple_reasoning.json',
        constructions_len
    )